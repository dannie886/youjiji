...
if($sm['config']['currency'] == 'NGN'){
    $EUR_price = $packageArray[$package]['price'] * 0.0025;
    $USD_price = $packageArray[$package]['price'] * 0.0028;
    $INR_price = $packageArray[$package]['price'] * 0.2;
    $TRY_price = $packageArray[$package]['price'] * 0.017;
    $NGN_price = $packageArray[$package]['price'];
} else {
    if($sm['config']['currency'] == 'USD' || $sm['config']['currency'] == 'EUR'){
        $req_url = 'https://api.exchangerate-api.com/v4/latest/'.$sm['config']['currency'];
        $response_json = file_get_contents($req_url);
        if(false !== $response_json) {
            try {
                $response_object = json_decode($response_json);
                $base_price = $packageArray[$package]['price'];
                ...
                $TRY_price = round(($base_price * $response_object->rates->TRY), 2);   
            }
            catch(Exception $e) {}
        } else {//realtime conversion failed show normal price to all
            $EUR_price = $packageArray[$package]['price'];
            $USD_price = $packageArray[$package]['price'];
            $INR_price = $packageArray[$package]['price'];
            $TRY_price = $packageArray[$package]['price'];            
        }        
        $NGN_price = $packageArray[$package]['price'] * 363.50;
    } else {
        $EUR_price = $packageArray[$package]['price'];
        $USD_price = $packageArray[$package]['price'];
        $INR_price = $packageArray[$package]['price'];
                            ...
                            <?php 
                                ...
                                <div class="lw-iyzico-form card mb-3" id="cardCheckoutForm">
                                    <div class="card-header">
                                        <h5 class="card-title display-td"><?= $sm['lang'][870]['text']; ?></h5>
